...
if __name__ == '__main__':
    ...
    filter_thres = 20
    support_thres = 50
    input_file_path = 'data/test_task2.csv'
    output_file_path = 'data/output_test_task2.txt'
    ...
    ###########
    # preprocess tafeng data to intermediate file
    ###########
    rdd_raw = sc.textFile(input_file_path)
    rdd_header = rdd_raw.first()
    rdd_data = rdd_raw.filter(lambda item: item != rdd_header)

    intermediate_file_path = 'data/customer_product_test.csv'
    rdd_preprocess = rdd_data.map(lambda item: item.replace('"', '').split(',')).\
        map(lambda item: (item[0] + '-' + str(int(item[1])), str(int(item[5]))))
    # write intermediate file
    with open(intermediate_file_path, 'w+') as o:
        header_list = ['DATE-CUSTOMER_ID', 'PRODUCT_ID']
        o.write(','.join(str(hd) for hd in header_list) + '\n')
        o.write('\n'.join(','.join(str(item) for item in tup) for tup in rdd_preprocess.collect()))
        o.close()


    # read data from intermediate file
    rdd_im = sc.textFile(intermediate_file_path)
    rdd_header_im = rdd_im.first()
    rdd_data_im = rdd_im.filter(lambda item: item != rdd_header_im)


    rdd_basket = rdd_data_im.map(lambda item: (item.split(',')[0], item.split(',')[1])). \
        groupByKey().map(lambda item: sorted(list(set(list(item[1]))))).\
        filter(lambda item: len(item) > filter_thres)
    count_all_baskets = rdd_basket.count()

    # SON phase1: get subset's candidates using PCY
    rdd_candidates = rdd_basket.mapPartitions(
        lambda partition: find_candidate_itemsets_pcy_phase1(partition, count_all_baskets, support_thres)). \
    ...
    rdd_frequent = rdd_basket.mapPartitions(
        lambda partition: count_candidate_phase2(partition, list_candidates_flat)). \
        reduceByKey(lambda x, y: x + y). \
