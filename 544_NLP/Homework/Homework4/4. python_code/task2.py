...
if __name__ == '__main__':
    ...
    # initialize the NN
    model = model_BiLSTM(len(dict_ner2idx))
    ...
    best_sd = train(20, model, train_loader, dev_loader, optimizer, criterion, \
                    X_train_tensor, y_train_tensor, X_dev_tensor, y_dev_tensor, print_every=1)

    # report scores of best model
    model_best = model_BiLSTM(len(dict_ner2idx))
    model_best.load_state_dict(best_sd)

    train_acc, train_prec, train_rec, train_f1, _ = report_scores(model_best(X_train_tensor), y_train_tensor)
    dev_acc, dev_prec, dev_rec, dev_f1, _ = report_scores(model_best(X_dev_tensor), y_dev_tensor)
